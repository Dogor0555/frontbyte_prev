...
export default function RetencionDTE07View( { user, hasHaciendaToken, haciendaStatus } ) {
  ...
  const handleViewJSON = async (iddtefactura) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/retencion/${iddtefactura}/descargar-json`, {
        method: 'GET',
        credentials: 'include',
      });
  ...
  const handleDownloadJSON = async (iddtefactura) => {
    try {
      ...
      let filename = `retencion-DTE07-${iddtefactura}.json`;
                    ...
                    <div className="bg-gray-50 px-3 py-2 flex flex-wrap gap-1 justify-between border-t border-gray-100">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          title={!(factura.documentofirmado && factura.documentofirmado !== "null") ? "No se puede ver: Retención no firmada" : "Ver JSON formateado"}
                        >
                          {loadingJson === factura.iddtefactura ? (
                            <>
