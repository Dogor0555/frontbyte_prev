...
export default function EditarCompraModal({ isOpen, onClose, compraId, onSuccess }) {
    ...
    const cargarCompra = async () => {
        try {
            setLoading(true);
            const response = await fetch(`${API_BASE_URL}/compras/getDetallesCompra/${compraId}`, {
                credentials: "include"
            });
            ...
            setFormData({
                ...
                nombre_proveedor: data.compra.nombre_proveedor || data.proveedor?.nombre || "",
            ...
            if (data.productos && data.productos.length > 0) {
                setDetalles(data.productos.map(p => ({
                ...
                {loading ? (
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-6">
                        ...
                        <div className="bg-gray-50 rounded-lg p-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input type="date" name="fecha" value={formData.fecha} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md" required />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Fecha Emisión</label>
                                    <input type="date" name="fecha_emision" value={formData.fecha_emision} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md" required />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Proveedor</label>
                                    <select name="proveedor_id" value={formData.proveedor_id} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md" required>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="numero_documento" value={formData.numero_documento} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md" required />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Tipo Documento</label>
                                    <select name="tipo_documento" value={formData.tipo_documento} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="nrc" value={formData.nrc} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-50 rounded-lg p-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-4">
                                <div className="md:col-span-5 relative">
                                    <input type="text" placeholder="Buscar producto..." value={productSearch} onChange={(e) => {
                                    ...
                                    {productSearch && !selectedProduct && filteredProducts.length > 0 && (
                                        <div className="absolute z-10 w-full bg-white border border-gray-300 mt-1 rounded-md shadow-lg max-h-48 overflow-y-auto">
                                            {filteredProducts.map(prod => (
                                                <div key={prod.id} className="px-4 py-2 hover:bg-blue-50 cursor-pointer text-sm" onClick={() => handleSelectProduct(prod)}>
                                ...
                                <div className="md:col-span-2">
                                    <input type="number" placeholder="Cantidad" value={addQuantity} onChange={(e) => setAddQuantity(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md" min="0.01" step="0.01" />
                                </div>
                                <div className="md:col-span-3">
                                    <input type="number" placeholder="Precio" value={addPrice} onChange={(e) => setAddPrice(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md" min="0" step="0.01" />
                                </div>
                                <div className="md:col-span-2">
                                    <button type="button" onClick={handleAddDetail} disabled={!selectedProduct} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-md flex items-center justify-center disabled:opacity-50">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {detalles.map((item, idx) => (
                                            <tr key={idx}>
                                                ...
                                                <td className="px-4 py-2 text-sm text-right">${item.precio_unitario.toFixed(2)}</td>
                        ...
                        <div className="bg-gray-50 rounded-lg p-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="exentas_internas" value={formData.exentas_internas} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md" step="0.01" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-600">Exentas Importación</label>
                                    <input type="number" name="exentas_importaciones" value={formData.exentas_importaciones} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md" step="0.01" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-600">Gravadas Internas</label>
                                    <input type="number" name="gravadas_internas" value={formData.gravadas_internas} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md" step="0.01" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-600">Gravadas Importación</label>
                                    <input type="number" name="gravadas_importaciones" value={formData.gravadas_importaciones} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md" step="0.01" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-600">Crédito Fiscal</label>
                                    <input type="number" name="credito_fiscal" value={formData.credito_fiscal} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md" step="0.01" />
                                </div>
                                <div className="md:col-span-2">
                                    <label className="block text-sm font-medium text-gray-600">Total</label>
                                    <div className="text-2xl font-bold text-green-600">${parseFloat(formData.total_compras || 0).toFixed(2)}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-between pt-4">
                            <button type="button" onClick={handleDelete} disabled={deleting} className="px-6 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md font-medium flex items-center disabled:opacity-50">
                                {deleting ? (
                                    <>
                            ...
                            <div className="flex space-x-3">
                                <button type="button" onClick={onClose} className="px-6 py-2 bg-gray-300 hover:bg-gray-400 text-gray-800 rounded-md font-medium">
                                    Cancelar
                                </button>
                                <button type="submit" disabled={saving} className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md font-medium flex items-center disabled:opacity-50">
                                    {saving ? (
                                        <>
                                            <div className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full mr-2"></div>
                                            Guardando...
