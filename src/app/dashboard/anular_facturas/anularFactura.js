...
export default function AnularFacturaView({ user, hasHaciendaToken, haciendaStatus }) {
  ...
  const handleAnularFactura = async (facturaId, motivo) => {
    ...
    try {
      ...
      addToast("Factura invalidada exitosamente", "success");
      setShowModal(false);
      setMotivoAnulacion("");
    } catch (error) {
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="ml-3">
                      <p className="text-sm text-yellow-700">
                        <strong>Nota:</strong> Solo puedes invalidar facturas que hayan sido transmitidas en las últimas 32 horas y que no estén ya invalidadas.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {facturasOrdenadas.length === 0 && (
                  <div className="text-center py-10">
                    ...
                    <p className="text-gray-500 mt-1">
                      {facturas.length === 0 
                        ? 'Todas las facturas transmitidas en las últimas 32 horas aparecerán aquí' 
