...
export default function AnexoSujetoExcluidoInvalidadasView({ user, hasHaciendaToken, haciendaStatus }) {
  ...
  const fetchDocumentos = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/anexo-sujeto-excluido-invalidadas?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`,
        {
          credentials: "include"
        }
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      datosParaExportar.forEach((doc, index) => {
        ...
        const fillColor = isEvenRow ? 'FFF2DCDB' : 'FFFFFFFF';
      ...
      const datosResumen = [
        ['ANEXO DE SUJETO EXCLUIDO INVALIDADAS - RESUMEN', ''],
        ['', ''],
      ...
      link.href = url;
      link.download = `anexo-sujeto-excluido-invalidadas-${fechaInicio}-a-${fechaFin}.xlsx`;
    ...
    try {
      ...
      chunks.forEach((chunk, pageIndex) => {
        ...
        if (pageIndex === 0) {
          ...
          doc.text("ANEXO DE SUJETO EXCLUIDO INVALIDADAS", pageWidth / 2, 15, { align: "center" });
      ...
      doc.save(`anexo-sujeto-excluido-invalidadas-${fechaInicio}-a-${fechaFin}.pdf`);
          ...
          <div className="p-6">
            <div className="max-w-full mx-auto">
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6">
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">Anexo de Sujeto Excluido Invalidadas</h1>
