...
export default function AnexoConsumidorInvalidadasView({ user, hasHaciendaToken, haciendaStatus }) {
  ...
  const fetchDocumentos = async () => {
    try {
      ...
      if (!response.ok) throw new Error("Error al cargar documentos invalidados");
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet('Anexo Invalidadas');

      // Definir columnas basadas en los campos del endpoint
      worksheet.columns = [
      ...
      // Estilo para la cabecera - ROJO con letras BLANCAS para invalidadas
      ...
      headerRow.eachCell((cell, colNumber) => {
        ...
        cell.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: 'FFC00000' } // Rojo para invalidadas
        };
        cell.alignment = { 
          vertical: 'middle', 
          horizontal: 'center',
      ...
      datosParaExportar.forEach((doc, index) => {
        ...
        const fillColor = isEvenRow ? 'FFF2DCDB' : 'FFFFFFFF'; // Rojo muy claro para filas alternadas
        ...
        row.eachCell((cell, colNumber) => {
          if (cell.value !== null && cell.value !== undefined && cell.value !== '') {
            ...
            // Formato numérico para columna de total
            if (colNumber === 13) {
              cell.numFmt = '#,##0.00';
            ...
            if (colNumber >= 3 && colNumber <= 9 && colNumber !== 13) {
      ...
      const datosResumen = [
        ['ANEXO DE CONSUMIDOR FINAL INVALIDADAS - RESUMEN', ''],
        ['', ''],
        ...
        ['Monto Total Invalidado:', documentos.reduce((sum, doc) => sum + (doc.total || 0), 0)],
        ['', ''],
        ['DISTRIBUCIÓN POR TIPO'],
        ['Facturas:', documentos.filter(doc => doc.tipo_documento?.includes('FACTURA')).length],
        ['Notas de Crédito:', documentos.filter(doc => doc.tipo_documento?.includes('CRÉDITO')).length],
        ['Notas de Débito:', documentos.filter(doc => doc.tipo_documento?.includes('DÉBITO')).length],
        ['Comprobantes CF:', documentos.filter(doc => doc.tipo_documento?.includes('COMPROBANTE')).length]
      ];
      ...
      datosResumen.forEach((fila, rowIndex) => {
        ...
        row.eachCell((cell, colNumber) => {
          if (cell.value !== null && cell.value !== undefined && cell.value !== '') {
            ...
            if (rowIndex === 0 || rowIndex === 5 || rowIndex === 9) {
              if (colNumber === 1) {
                cell.font = { 
                  color: { argb: 'FFFFFFFF' }, 
                  bold: true,
      ...
      link.href = url;
      link.download = `anexo-consumidor-invalidadas-${fechaInicio}-a-${fechaFin}.xlsx`;
    ...
    try {
      ...
      chunks.forEach((chunk, pageIndex) => {
        ...
        if (pageIndex === 0) {
          ...
          doc.text("ANEXO DE CONSUMIDOR FINAL INVALIDADAS", pageWidth / 2, 15, { align: "center" });
          ...
          doc.text(`Monto Total: ${formatCurrency(documentos.reduce((sum, doc) => sum + (doc.total || 0), 0))}`, margin + 70, 42);
        }
        ...
        autoTable(doc, {
          startY: startY,
          head: pageIndex === 0 ? [
            ['Resolución', 'Clase Doc', 'Desde Pre', 'Hasta Pre', 'Tipo Doc', 'Tipo Detalle', 'N° Serie', 'Desde', 'Hasta', 'Código Gen', 'Fecha Emi', 'N° Factura', 'Total']
          ] : [],
          body: tableData,
          theme: 'grid',
          headStyles: {
            fillColor: [192, 0, 0], // Rojo para invalidadas
            textColor: 255,
            fontStyle: 'bold',
            fontSize: 5
          },
      ...
      doc.save(`anexo-consumidor-invalidadas-${fechaInicio}-a-${fechaFin}.pdf`);
  ...
  // Columnas basadas en los campos del endpoint
  const columns = [
    { key: 'numero_resolucion', label: 'RESOLUCIÓN', width: '180px' },
    { key: 'clase_documento', label: 'CLASE DOCUMENTO', width: '250px' },
    { key: 'desde_preimpreso', label: 'DESDE PRE', width: '80px', align: 'center' },
    { key: 'hasta_preimpreso', label: 'HASTA PRE', width: '80px', align: 'center' },
    { key: 'tipo_documento', label: 'TIPO DOCUMENTO', width: '180px' },
    { key: 'tipo_detalle', label: 'TIPO DETALLE', width: '180px' },
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Cargando anexo de documentos invalidados...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="p-6">
            <div className="max-w-full mx-auto">
              ...
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6">
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">Anexo de Consumidor Final Invalidadas</h1>
                  <p className="text-gray-600 text-sm">
                    Registro de documentos DTE invalidados - Control tributario
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="text"
                        placeholder="Resolución, código, serie, factura..."
                        className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-gray-500 transition-colors"
                        value={searchTerm}
                        onChange={(e) => {
                    ...
                    <thead className="bg-red-50">
                      <tr>
                        {columns.map((column) => (
                          <th 
                            key={column.key}
                            className={`px-3 py-3 text-xs font-medium text-red-700 uppercase tracking-wider whitespace-nowrap ${
                          ...
                          {columns.map((column) => (
                            <td 
                              ...
                              className={`px-3 py-3 text-sm ${
                                ...
                                ['numero_resolucion', 'codigo_generacion', 'numero_serie', 'numero_factura'].includes(column.key)
                ...
                {documentosFiltrados.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-gray-700 mb-2">
                      No se encontraron documentos invalidados
                    </h3>
                    <p className="text-gray-500 text-sm">
                      {documentos.length === 0 
                        ? 'No hay documentos invalidados en el período seleccionado' 
