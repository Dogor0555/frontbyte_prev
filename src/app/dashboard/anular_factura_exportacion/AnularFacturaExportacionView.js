...
export default function AnularFacturaExportacionView({ user, hasHaciendaToken, haciendaStatus }) {
  ...
  const handleAnularFactura = async (facturaId, motivo) => {
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        const errorMessage = data.detalles || data.error || "Error al invalidar la factura de exportación";
  ...
  const handleGeneratePDF = async (facturaId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/exportacion/${facturaId}/descargar-pdf`, {
        credentials: "include"
      });
      ...
      let filename = `FEX-${facturaId}.pdf`;
  ...
  const handleDownloadJSON = async (iddtefactura) => {
    try {
      const response = await fetch(`http://localhost:3000/exportacion/${iddtefactura}/descargar-json`, {
        method: 'GET',
        credentials: 'include',
      });
      ...
      const disposition = response.headers.get("Content-Disposition");
      let filename = `exportacion-${iddtefactura}.json`;
      if (disposition && disposition.includes("filename=")) {
        filename = disposition
          .split("filename=")[1]
          .replace(/"/g, "");
  ...
  const FacturaCard = ({ factura, esInvalidada = false }) => (
    ...
    >
      <div className={`p-3 ${esInvalidada ? 'bg-red-600' : 'bg-orange-600'} text-white`}>
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            ...
            <div>
              <span className="font-semibold text-xs block">FACT. EXPORTACIÓN</span>
      ...
      <div className="p-4">
        <div className="flex justify-between items-center mb-3 pb-2 border-b border-gray-100">
          <div className="flex items-center text-gray-600">
            <FaCalendarAlt className={`mr-1 ${esInvalidada ? 'text-red-500' : 'text-orange-500'} text-xs`} />
        ...
        <div className="mb-3">
          <div className="flex items-center text-gray-700 mb-1">
            <FaUser className={`mr-1 ${esInvalidada ? 'text-red-500' : 'text-orange-500'} text-xs`} />
        ...
        <div className="border-t border-gray-100 pt-2">
          <div className="flex justify-between items-center">
            ...
            <div className={`text-lg font-bold ${esInvalidada ? 'text-red-600' : 'text-orange-600'}`}>
      ...
      <div className={`px-3 py-3 flex flex-wrap gap-2 justify-between border-t ${
        esInvalidada ? 'bg-red-50 border-red-100' : 'bg-orange-50 border-orange-100'
      }`}>
        <button
              ...
              pages.push(
                <button
                  ...
                  className={`px-3 py-1 border-t border-b ${
                    1 === currentPage ? 'bg-orange-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  1
                </button>
            ...
            for (let i = startPage; i <= endPage; i++) {
              pages.push(
                <button
                  ...
                  className={`px-3 py-1 border-t border-b ${
                    i === currentPage ? 'bg-orange-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  {i}
                </button>
            ...
            if (endPage < totalPages) {
              ...
              pages.push(
                <button
                  ...
                  className={`px-3 py-1 border-t border-b ${
                    totalPages === currentPage ? 'bg-orange-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  {totalPages}
                </button>
            ...
            <div className="max-w-6xl mx-auto">
              <div className="mb-8">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                  <div>
                    <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Invalidar Facturas de Exportación</h1>
                    <p className="text-gray-600">
                      {facturas.length} {facturas.length === 1 ? "documento invalidable" : "documentos invalidables"} 
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    <div className="relative w-full md:w-64">
                      ...
                      <input
                        type="text"
                        placeholder="Buscar documentos invalidables..."
                    ...
                    <select
                      ...
                      className="px-3 py-2 border rounded-lg focus:ring-2 bg-white focus:ring-orange-500 focus:border-orange-500 text-gray-700"
                    >
                ...
                {facturasOrdenadas.length === 0 && (
                  <div className="text-center py-10">
                    ...
                    <h3 className="text-lg font-medium text-gray-700">
                      {facturas.length === 0 ? 'No hay documentos invalidables' : 'No se encontraron coincidencias'}
                    </h3>
              ...
              <div className="border-t border-gray-200 pt-8">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                  <div>
                    <h2 className="text-xl md:text-2xl font-bold text-gray-800">Documentos Invalidados</h2>
                    <p className="text-gray-600">
                      {facturasInvalidadas.length} {facturasInvalidadas.length === 1 ? "documento invalidado" : "documentos invalidados"}
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    <div className="relative w-full md:w-64">
                      ...
                      <input
                        type="text"
                        placeholder="Buscar documentos invalidados..."
                ...
                {facturasInvalidadasOrdenadas.length === 0 && (
                  <div className="text-center py-10">
                    ...
                    <h3 className="text-lg font-medium text-gray-700">
                      No hay documentos invalidados
                    </h3>
                    <p className="text-gray-500 mt-1">
                      Los documentos que invalides aparecerán en esta sección
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              ...
              <p className="text-sm text-gray-500 mb-4">
                ¿Estás seguro de que deseas invalidar el documento #{facturaSeleccionada?.numerofacturausuario}?
              ...
              <div className="flex justify-end space-x-3">
                ...
                >
                  {anulando ? "Invalidando..." : "Confirmar Invalidación"}
                </button>
              </div>
            </div>
          </div>
