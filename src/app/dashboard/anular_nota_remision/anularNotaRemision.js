...
export default function AnularNotaRemisionView({ user, hasHaciendaToken, haciendaStatus }) {
  ...
  useEffect(() => {
    const fetchFacturas = async () => {
      try {
        ...
        alert("Error al cargar las notas de remisión: " + error.message);
        setFacturas([]);
        setFacturasInvalidadas([]);
      } finally {
        setLoading(false);
  ...
  const handleAnularFactura = async (facturaId, motivo) => {
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        const errorMessage = data.detalles || data.error || "Error al anular nota de remisión";
      ...
      alert("Nota de remisión invalidada exitosamente");
      setShowModal(false);
      setMotivoAnulacion("");
    } catch (error) {
  ...
  const handleGeneratePDF = async (facturaId) => {
    ...
    try {
      ...
      let filename = `REM-${facturaId}.pdf`;
  ...
  const handleDownloadJSON = async (iddtefactura) => {
    try {
      ...
      const disposition = response.headers.get("Content-Disposition");
      let filename = `remision-${iddtefactura}.json`;
      if (disposition && disposition.includes("filename=")) {
        filename = disposition
          .split("filename=")[1]
          .replace(/"/g, "");
  ...
  const handleViewDetails = (facturaId) => {
    // router.push(`/dashboard/nota_remision/${facturaId}`);
  };
      ...
      <div className={`p-3 ${esInvalidada ? 'bg-red-600' : 'bg-blue-600'} text-white`}>
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            ...
            <div>
              <span className="font-semibold text-xs block">NOTA DE REMISIÓN</span>
      ...
      <div className="p-4">
        ...
        <div className="mb-3">
          ...
          <p className="text-gray-900 text-sm font-medium truncate pl-3">
            {factura.nombrecibe || 'Receptor no especificado'}
          </p>
          {factura.docuentrega && (
            ...
            <div className="max-w-6xl mx-auto">
              <div className="mb-8">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                  <div>
                    <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Invalidar Notas de Remisión</h1>
                    <p className="text-gray-600">
                      {facturas.length} {facturas.length === 1 ? "nota de remisión invalidable" : "notas de remisión invalidables"} 
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    <div className="relative w-full md:w-64">
                      ...
                      <input
                        type="text"
                        placeholder="Buscar notas de remisión invalidables..."
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="ml-3">
                      <p className="text-sm text-yellow-700">
                        <strong>Nota:</strong> Solo puedes invalidar notas de remisión que hayan sido transmitidas en las últimas 24 horas y que no estén ya invalidadas.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {facturasOrdenadas.length === 0 && (
                  <div className="text-center py-10">
                    ...
                    <h3 className="text-lg font-medium text-gray-700">
                      {facturas.length === 0 ? 'No hay notas de remisión invalidables' : 'No se encontraron coincidencias'}
                    </h3>
                    <p className="text-gray-500 mt-1">
                      {facturas.length === 0 
                        ? 'Todas las notas de remisión transmitidas en las últimas 24 horas aparecerán aquí' 
              ...
              <div className="border-t border-gray-200 pt-8">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                  <div>
                    <h2 className="text-xl md:text-2xl font-bold text-gray-800">Notas de Remisión Invalidadas</h2>
                    <p className="text-gray-600">
                      {facturasInvalidadas.length} {facturasInvalidadas.length === 1 ? "nota de remisión invalidada" : "notas de remisión invalidadas"}
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    <div className="relative w-full md:w-64">
                      ...
                      <input
                        type="text"
                        placeholder="Buscar notas de remisión invalidadas..."
                ...
                {facturasInvalidadasOrdenadas.length === 0 && (
                  <div className="text-center py-10">
                    ...
                    <h3 className="text-lg font-medium text-gray-700">
                      No hay notas de remisión invalidadas
                    </h3>
                    <p className="text-gray-500 mt-1">
                      Las notas de remisión que invalides aparecerán en esta sección
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              ...
              <p className="text-sm text-gray-500 mb-4">
                ¿Estás seguro de que deseas invalidar la nota de remisión #{facturaSeleccionada?.numerofacturausuario}?
              ...
              <div className="flex justify-end space-x-3">
                ...
                >
                  {anulando ? "Invalidando..." : "Invalidar Nota de Remisión"}
                </button>
              </div>
            </div>
          </div>
