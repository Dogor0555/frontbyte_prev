...
export default function FacturasExcluidasView({ user, hasHaciendaToken, haciendaStatus, initialFacturas }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/sujeto-excluido?${params}`, {
        credentials: "include"
      });
  ...
  const handleTransmitir = async (facturaId) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Error en transmisión');
      }
  ...
  const handleNuevaFactura = () => {
    router.push('/dashboard/facturas-excluidas/nueva');
  };
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-4 md:p-6">
            <div className="max-w-6xl mx-auto">
              {/* Header con información de sujeto excluido */}
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                  ...
                  <p className="text-gray-600">
                    {meta.total} {meta.total === 1 ? "documento" : "documentos"} excluidos registrados
                    ...
                    <div className="p-4">
                      ...
                      <div className="mb-3">
                        ...
                        <p className="text-gray-900 text-sm font-medium truncate pl-3">
                          {factura.nombrecibe || factura.nombrentrega || 'Cliente no especificado'}
                        </p>
                        {factura.docuentrega && (
                ...
                <div className="flex justify-center mt-6">
                  <nav className="inline-flex rounded-md shadow">
                      ...
                      if (startPage > 1) {
                        pages.push(
                          <button
                            key={1}
                            onClick={() => setPage(1)}
                            className={`px-3 py-1 border-t border-b ${1 === page ? 'bg-green-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
                              }`}
                          >
                            1
                          </button>
                      ...
                      for (let i = startPage; i <= endPage; i++) {
                        pages.push(
                          <button
                            key={i}
                            onClick={() => setPage(i)}
                            className={`px-3 py-1 border-t border-b ${i === page ? 'bg-green-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
                              }`}
                          >
                            {i}
                          </button>
                      ...
                      if (endPage < meta.pages) {
                        ...
                        pages.push(
                          <button
                            ...
                            className={`px-3 py-1 border-t border-b ${meta.pages === page ? 'bg-green-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
                              }`}
                          >
                            {meta.pages}
                          </button>
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-r-md border ${page === meta.pages
                          ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                          : 'bg-white text-gray-700 hover:bg-gray-50'
                        }`}
                    >
              ...
              {!loading && rows.length === 0 && (
                <div className="text-center py-10">
                  ...
                  <h3 className="text-lg font-medium text-gray-700">
                    {meta.total === 0 ? 'No hay facturas de sujeto excluido registradas' : 'No se encontraron coincidencias'}
                  </h3>
                  <p className="text-gray-500 mt-1">
                    {meta.total === 0 ? 'Comienza creando tu primera factura de sujeto excluido' : 'Intenta con otros términos de búsqueda o rango de fechas'}
                  </p>
                  {hayFiltros && (
                    <button
                      onClick={limpiarFiltros}
