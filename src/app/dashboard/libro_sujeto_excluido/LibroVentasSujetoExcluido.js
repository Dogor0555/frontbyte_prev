//src/app/dashboard/libro_sujeto_excluido/LibroVentasSujetoExcluido.js
  ...
  const fetchLibro = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/libro-ventas-sujeto-excluido-rango?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`,
        {
          credentials: "include"
        }
      );
...
const handleExportExcel = async () => {
  ...
  try {
    ...
    worksheet.getCell("A2").value =
      "LIBRO DE VENTAS A SUJETO EXCLUIDO";
    ...
    link.download =
      `libro-ventas-sujeto-excluido-${fechaInicio}-a-${fechaFin}.xlsx`;
    ...
    try {
      ...
      chunks.forEach((chunk, pageIndex) => {
        ...
        if (pageIndex === 0) {
          ...
          doc.text("LIBRO DE VENTAS A SUJETO EXCLUIDO", pageWidth / 2, 15, { align: "center" });
      ...
      doc.save(`libro-ventas-sujeto-excluido-${fechaInicio}-a-${fechaFin}.pdf`);
  ...
  const descargarCSV = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/anexo-sujeto-excluido/csv?${params.toString()}`, {
        credentials: "include",
      });
      ...
      a.href = url;
      a.download = `Libro_Ventas_Sujeto_Excluido_${fechaInicio}_${fechaFin}.csv`;
      a.click();
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Cargando libro de ventas a sujeto excluido...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="p-6">
            <div className="max-w-full mx-auto">
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6">
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">Libro de Ventas a Sujeto Excluido</h1>
                  <p className="text-gray-600 text-sm">
                    Registro diario de ventas a sujeto excluido - Vista simplificada
                  </p>
                </div>
