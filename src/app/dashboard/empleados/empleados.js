...
import { FaSearch, FaBars, FaPlus, FaTimes, FaEdit, FaTrash, FaFilePdf, FaSave, FaTimes as FaClose, FaUser, FaEye, FaEyeSlash, FaToggleOn, FaToggleOff } from "react-icons/fa";
...
export default function Empleados({ initialEmpleados = [], user }) {
    ...
    const [filteredEmpleados, setFilteredEmpleados] = useState(initialEmpleados || []);
    ...
    const validateForm = () => {
        ...
        if (formData.numerodocumento.length > LIMITES.NUMERODOCUMENTO) {
            setErrorMessage(`El número de documento no puede exceder los ${LIMITES.NUMERODOCUMENTO} caracteres.`);
            return false;
        }
        ...
        if (!showEditModal && (!formData.contrasena || formData.contrasena.length < 6)) {
    ...
    useEffect(() => {
        const results = empleados && Array.isArray(empleados) ? empleados.filter(empleado =>
            empleado.idempleado.toString().includes(searchTerm) ||
            empleado.numerodocumento.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const handleUpdateEmpleado = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/empleados/update/${formData.idempleado}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Cookie: document.cookie,
    ...
    const handleToggleEstado = async (empleadoId, nuevoEstado) => {
        try {
            const response = await fetch(`http://localhost:3000/empleados/toggle-estado/${empleadoId}`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                    Cookie: document.cookie,
    ...
    const handleDeleteEmpleado = async (empleadoId) => {
        try {
            const response = await fetch(`http://localhost:3000/empleados/delete/${empleadoId}`, {
                method: "DELETE",
                headers: {
                    Cookie: document.cookie,
                },
    ...
    const handleChangePassword = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/empleados/change-password/${selectedEmpleado.idempleado}`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Cookie: document.cookie,
                ...
                <div className="flex-1 flex flex-col">
                    ...
                    <div className="flex-1 p-4 md:p-6">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                            <div className="flex items-center">
                                <h2 className="text-2xl font-bold text-gray-800 tracking-tight">Empleados</h2>
                            </div>
                            <button
                            ...
                            <div className="overflow-x-auto rounded-lg shadow">
                                <table className="min-w-full bg-white border border-gray-200">
                                    ...
                                    <tbody className="divide-y divide-gray-200">
                                        {filteredEmpleados && Array.isArray(filteredEmpleados) && filteredEmpleados.slice(0, 10).map((empleado) => (
                                            <tr key={empleado.idempleado} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                    {getTipoDocumentoNombre(empleado.tipodocumento)}: {empleado.numerodocumento}
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{empleado.correo}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{empleado.rol}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                    ...
                                                    <button
                                                        onClick={() => handleToggleEstado(empleado.idempleado, !empleado.estado)}
                                                        className={`mr-2 ${empleado.estado ? 'text-red-600 hover:text-red-800' : 'text-green-600 hover:text-green-800'}`}
                                                        aria-label={empleado.estado ? "Desactivar" : "Activar"}
                                                    >
                                                        {empleado.estado ? <FaToggleOff /> : <FaToggleOn />}
                                                    </button>
                                                    <button
                                                        onClick={() => handleDeleteClick(empleado.idempleado)}
                        ...
                        <div className="md:hidden">
                            <div className="space-y-4">
                                {filteredEmpleados && Array.isArray(filteredEmpleados) && filteredEmpleados.slice(0, 10).map((empleado) => (
                                    <div key={empleado.idempleado} className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
                                        <div className="flex justify-between items-start">
                                            <h3 className="text-lg font-medium text-gray-900">{empleado.nombre}</h3>
                                        ...
                                        <div className="mt-4 space-y-2">
                                            <div className="flex justify-between">
                                                ...
                                                <span className="text-sm text-gray-900">{getTipoDocumentoNombre(empleado.tipodocumento)}: {empleado.numerodocumento}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-sm font-medium text-gray-500">Correo:</span>
                                                <span className="text-sm text-gray-900">{empleado.correo}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-sm font-medium text-gray-500">Rol:</span>
                                                <span className="text-sm text-gray-900">{empleado.rol}</span>
                                            </div>
                                            <div className="flex justify-between">
            ...
            {showAddModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-lg shadow-lg max-w-md w-full max-h-screen overflow-y-auto">
                        ...
                        <form onSubmit={handleSaveNewEmpleado} className="px-6 py-4 space-y-4">
                            <div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        required
                                    />
                                    <p className="text-xs text-gray-500 mt-1">{formData.numerodocumento.length}/{LIMITES.NUMERODOCUMENTO} caracteres</p>
                                </div>
                            </div>
            ...
            {showEditModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-lg shadow-lg max-w-md w-full max-h-screen overflow-y-auto">
                        ...
                        <form onSubmit={handleUpdateEmpleado} className="text-black px-6 py-4 space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Nombre</label>
                                <input
                                    type="text"
                            ...
                            <div className="flex items-center">
                                <input
                                    ...
                                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                />
                                <label htmlFor="estadoEdit" className="ml-2 block text-sm text-gray-900">
                                    Estado activo
                                </label>
                            </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-lg shadow-lg max-w-md w-full">
                        ...
                        <div className="px-6 py-4">
                            <p className="text-sm text-gray-600">¿Estás seguro de que deseas eliminar este empleado? Esta acción no se puede deshacer.</p>
                        </div>
            ...
            {showCancelConfirmModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-lg shadow-lg max-w-md w-full">
                        ...
                        <div className="px-6 py-4">
                            <p className="text-sm text-gray-600">Tienes cambios sin guardar. ¿Estás seguro de que deseas cancelar?</p>
                        </div>
                        <div className="flex justify-end space-x-3 px-6 py-4 bg-gray-50 rounded-b-lg">
                            <button
